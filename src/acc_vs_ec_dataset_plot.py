...
# python3 -m src.acc_vs_ec_dataset_plot
...
def ec_vs_accuracy_dataset():
    for dataset in df.Dataset.unique():
        ...
        sns.scatterplot(data=data, x="EC", y="MAE", hue="Method", style="Regressor", s=350)
        ...
        plt.title(f"Error Consistency vs MAE for the {dataset} dataset ", fontsize=25)
        plt.xticks(fontsize=25)
        plt.yticks(fontsize=25)
        plt.savefig(PLOT_OUTPUT_PATH / 'new_medical_plots' / f"MAE_{dataset}_only_conf.png", bbox_inches='tight')
        plt.clf()
